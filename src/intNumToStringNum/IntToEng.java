...
public class IntToEng {
	...
	public static String translateEng(int n) {
		...
		for(int j=3;j>=0;j--) {
			...
			else if(i>100)str=prettyStrAdd(str,OneToTen[(i/100)-1]+" Hundreds");

			int k=(i/10)%10;//下3桁のうちの2桁目の数字を取り出す
			if(k==1) {//10~19のとき
				str=prettyStrAdd(str,TenToNineteen[(i%100)-10]);
			}else if(k>0) {//20~99のとき
				str=prettyStrAdd(str,Ten_digit[k-2]);
				if(i%10!=0) str=prettyStrAdd(str,OneToTen[(i%10)-1]);
			}else {//0~9のとき、ただし0は何も表示しない。
				if(i%10!=0) str=prettyStrAdd(str,OneToTen[(i%10)-1]);
			}
			if(i==1)stack.push(prettyStrAdd(str,digit[3-j]).substring(0, digit[3-j].length()+3));
			else {stack.push(prettyStrAdd(str,digit[3-j]));}
		}
	...
	public static String stackAdd(Stack<String> stack) {
		String str="";
		while(!stack.empty()) {
			str=prettyStrAdd(str,stack.pop());
			}
		return str;
	}
	public static String prettyStrAdd(String base,String add) {
